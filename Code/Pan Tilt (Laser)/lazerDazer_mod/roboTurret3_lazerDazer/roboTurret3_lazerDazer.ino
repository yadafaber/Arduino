...
#include <VarSpeedServo.h>  //include the Variable Speed servo library for working with servo objects
...
const int PAN_MIN = 90;     // minimum pan servo range, value should be between 0 and 180 and lower than PAN_MAX
const int PAN_MAX = 91;    // maximum pan servo range, value should be between 0 and 180 and more than PAN_MIN

const int TILT_MIN = 50;    // minimum tilt servo range, value should be between 0 and 180 and lower than TILT_MAX
const int TILT_MAX = 90;   // maximum tilt servo range, value should be between 0 and 180 and more than TILT_MIN

const int PAN_SPEED_MIN = 10;     // minimum pan servo speed, value should be between 1 and 255 and lower than PAN_SPEED_MAX
const int PAN_SPEED_MAX = 255;    // maximum pan servo speed, value should be between 1 and 255 and higher than PAN_SPEED_MIN

const int TILT_SPEED_MIN = 254;    // minimum tilt servo speed, value should be between 1 and 255 and lower than TILT_SPEED_MAX
const int TILT_SPEED_MAX = 255;    // maximum tilt servo speed, value should be between 1 and 255 and higher than TILT_SPEED_MAX

const int SPEED_MIN = 99;   // minimum interpolation speed (less than 5 not recommended)
const int SPEED_MAX = 100;   // maximum interpolation speed (Lower SPEED_MAX values mean that the program will cycle more quickly, leading to an increased occurance of pausing. Adjust ROLL_MAX accordingly)
                       
const int ROLL_MAX = 400;    // Roll the dice for random pause. Lower value = more frequent pauses, higher value = less frequent pauses

const int PAUSE_MIN = 500;  // define minimum pause length in mS. 1000 = 1 second
const int PAUSE_MAX = 1550; // define maximum pause length in mS.


VarSpeedServo panServo, tiltServo;  // create servo objects to control the pan and tilt servos
...
{ 
  ...
  panServo.write(panValue, 0);  //sets the pan servo position to the default 'home' value, moves to home at full speed
  tiltServo.write(tiltValue, 0); //sets the tilt servo position to the default 'home' value, moves to home at full speed
...
{ 
  ...
  pan_speed = random(PAN_SPEED_MIN, PAN_SPEED_MAX);
  tilt_speed = random(TILT_SPEED_MIN, PAN_SPEED_MAX);
  ...
  */ 
  ...
  panServo.write(pan_goal, pan_speed);                    // sets the servo position and speed according to the scaled value 
  tiltServo.write(tilt_goal, pan_speed);                  // sets the servo position and speed according to the scaled value 
